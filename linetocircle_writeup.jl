...
# ╔═╡ 28b8b710-eafd-41e6-b43f-f0c72c5d2b22
begin
	using Plots
	using PlutoUI
	using Colors
	...
	using Distributions
	gr(bg = RGB(0, 0, 0))
	default(dpi = 150, framestyle = :box, fontfamily = "Computer Modern", label = false, grid = true)
end
...
Given $n$ points along a line segment, map those points onto a given circle using various algorithms. Benchmark each algorithm, and discuss the pros and cons.

Below is a visual depiction of the problem at hand to be solved:

"""

# ╔═╡ 7481805c-09b4-4e8d-aee0-7039f50052d5
function set_plot_settings!(fig)
	vline!(fig, [0], color = :white)
	hline!(fig, [0], color = :white)
	xaxis = Plots.get_axis(Plots.get_subplot(fig,1),:x)
	yaxis = Plots.get_axis(Plots.get_subplot(fig,1),:y)
	xaxis[:gridalpha] = 0.4
	yaxis[:gridalpha] = 0.4
	xaxis[:foreground_color_grid] = RGB(66/255, 228/255, 254/255)
	yaxis[:foreground_color_grid] = RGB(66/255, 228/255, 254/255)
end

# ╔═╡ 8e392836-ae08-46c2-b29a-74cb8753dd8c
TableOfContents()

# ╔═╡ 35bfb6a2-3d2e-4be8-97e0-5603066affa3
begin
	...
	function points(l::Line; npts = 300)
		x = range(l.x_range..., npts)
		return (x, l.fn.(x))
	end
...
function line_to_circle_animation(line::Line; nframes = 300, kwargs...)
	xmin, xmax = line.x_range
	line_xrange = range(xmin, xmax, 500)
	line_yrange = line.(line_xrange)

	θ = range(0, 2π, 500)
	f(x) = 2π * ( (x - xmin) / (xmax - xmin) )

	mapped_xrange = cos.(f.(line_xrange))
	mapped_yrange = sin.(f.(line_xrange))
	vx = (mapped_xrange .- line_xrange)
...
let
	...
	animation = line_to_circle_animation(L, nframes = 120, xlim = (-4, 4), ylim = (-4, 4), aspect_ratio = :equal)
...
md""" ### Method 1: Parameterize the Line for $\theta$

Let our sample line be of the form $y = \frac{\pi}{4}x + 3$. Let the circle the sample line has to be projected onto be centered at the origin.
"""
...
function line_to_circle_map_parametric(l::Line, x; r = 1)
	θ = 2π * ( (x - l.x_range[1])/(l.x_range[2] - l.x_range[1]) )
...
# ╔═╡ 96024f81-e021-4bb8-9266-8f668822809a
@benchmark line_to_circle_map_parametric_complex(L, x) setup=(x = rand(Uniform(L.x_range...)))

# ╔═╡ a971caf8-4f4a-4ecc-a14f-e7fd520aded1
md""" ### Method 2: Using the Argument of Each Point on the Line

Using the same sample line as in Method 1, the argument of the points on the line is used to map them onto a circle centered at the origin.
"""

# ╔═╡ 6bbc966b-c769-4e67-b14d-f71130713633
function line_to_theta_atan2(l::Line, x; r = 1)
	y = l(x)
	θ = atan(y, x)
	return r * cos(θ), r * sin(θ)
end
...
line_to_theta_atan2_complex(L; r = 1) = x -> line_to_theta_atan2_complex(L, x; r = r)

# ╔═╡ aa6c4fad-cdd2-4250-a98d-88a6430773ca
@benchmark line_to_theta_atan2(L, x) setup=(x = rand(Uniform(L.x_range...)))
...
let
	...
	mapped_points = line_to_theta_atan2(line).(line_xrange)
	mapped_xrange = (x -> x[1]).(mapped_points)
	mapped_yrange = (x -> x[2]).(mapped_points)
	vx = (mapped_xrange .- line_xrange) 
	vy = (mapped_yrange .- line_yrange)
...
manifest_format = "2.0"
project_hash = "012b683a01ea150241f61179245b66476b026f55"
